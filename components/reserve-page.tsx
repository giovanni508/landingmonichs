...
export function ReservePage() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#07111f] text-[#f4e6da]">
      <section className="relative isolate flex min-h-screen items-center px-4 py-4 sm:px-8 sm:py-8 lg:px-14">
        ...
        <div className="mx-auto grid w-full max-w-6xl items-center gap-4 sm:gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
          <div className="order-2 max-w-xl lg:order-1">
            <div className="mb-3 flex items-center gap-3 text-[10px] font-bold uppercase tracking-[0.3em] text-[#e8c5a6]"><span className="h-px w-8 bg-[#e8c5a6]/70" />Monichs · Super Early Bird</div>
            <h1 className="max-w-lg text-3xl font-semibold leading-[0.98] tracking-[-0.05em] text-[#f7eee8] sm:text-6xl">Your place in the first 50.</h1>
            <p className="mt-2 max-w-md text-xs leading-snug sm:text-lg text-[#f4e6da]/65 sm:text-lg">Reserve your Super Early Bird price for just $5. You&apos;ll secure the $1,400 launch offer before the campaign goes live.</p>
            <div className="mt-2 rounded-2xl border border-[#e8c5a6]/20 bg-white/[0.045] p-3 backdrop-blur-sm sm:p-6">
              <div className="flex items-end justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.25em] text-[#e8c5a6]">Reservation</p><p className="mt-2 text-3xl font-semibold tracking-tight">$5</p><p className="mt-1 hidden text-xs text-[#f4e6da]/45 sm:block">Applied toward your Super Early Bird purchase</p></div><div className="text-right"><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#f4e6da]/45">Only</p><p className="mt-1 text-2xl font-semibold text-[#e8c5a6]">50 spots</p><p className="mt-1 hidden text-xs text-[#f4e6da]/45 sm:block">available worldwide</p></div></div>
              <div className="mt-4 border-t border-[#e8c5a6]/15 pt-5"><label htmlFor="edition-number" className="flex items-center justify-between text-xs font-semibold text-[#f4e6da]/80">Choose your engraved number<span className="font-normal text-[#f4e6da]/40">1–50</span></label><select id="edition-number" value={number} onChange={(event) => setNumber(event.target.value)} className="mt-3 h-12 w-full rounded-lg border border-[#e8c5a6]/35 bg-[#07111f] px-4 text-sm font-semibold text-[#f4e6da] outline-none transition focus:border-[#e8c5a6] focus:ring-2 focus:ring-[#e8c5a6]/20">{Array.from({ length: 50 }, (_, index) => { const value = String(index + 1).padStart(2, "0"); return <option key={value} value={value}>#{value}</option> })}</select><p className="mt-2 hidden text-[11px] leading-relaxed text-[#f4e6da]/40 sm:block">Number selection is subject to availability and is confirmed after payment.</p></div>
          ...
          <div className="order-1 flex justify-center lg:order-2 lg:justify-end"><div className="relative w-full max-w-[34rem]"><div aria-hidden="true" className="absolute -inset-6 rounded-[2rem] bg-[#e8c5a6]/10 blur-3xl" /><div className="relative overflow-hidden rounded-[1.5rem] border border-[#e8c5a6]/20 bg-[#101d2e] shadow-[0_30px_100px_-30px_rgba(0,0,0,0.8)]"><img src="/images/watches-trio-detail.jpg" alt="Three Monichs Swiss Made watches" className="aspect-[4.8/1] w-full object-cover object-center sm:aspect-[4/5]" /><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#07111f] via-[#07111f]/45 to-transparent px-5 pb-5 pt-20"><p className="text-[10px] font-bold uppercase tracking-[0.28em] text-[#e8c5a6]">Swiss Made · Limited Edition</p><p className="mt-2 text-sm text-[#f4e6da]/75">Designed in Italy. Engineered in Switzerland.</p></div></div><div className="absolute -bottom-3 -left-3 rounded-full border border-[#e8c5a6]/35 bg-[#07111f]/90 px-4 py-2 text-[10px] font-bold uppercase tracking-[0.2em] text-[#e8c5a6] backdrop-blur-md sm:-left-5">Edition 01—50</div></div></div>
        </div>
      </section>
    </main>
  )
